...
# Load tsv file of Baltimore shipments and store as 'baltimore'
...
# Open baltimore in new window to examine - comment out once complete
#View(baltimore)
...
# The columns that are stored as <dbl> that don't need to be are REPORTER_ZIP, BUYER_ZIP, DRUG_CODE and TRANSACTION_ID - they are numerical values that represent non-numerical concepts, so we don't need to be able to perform operations on them. There aren't any <chr> fields that jump out as needing to be stored in another format.

# Missing values are represented as "null" in this set.
...
# Between 2006 and 2012, Newcare Home Health Services, located in the 3400 block of Sinclair Lane, received 6,133,600 opioid pills, nearly 2 million more than any other pharmacy in Baltimore City. In October 2006, Newcare's owners were indicted for selling hydrocodone pills under false prescriptions for non-medical purposes and for money laundering.
...
# McKesson Corporation, whose corporate headquarters are in Texas, distributed the most pills to Baltimore City between 2006 and 2012: 49,834,830 total pills.
...
baltimore %>%
  ...
  group_by(buyer_name,buyer_address1,buyer_address2,buyer_zip) %>%
  summarise(
    shipments = n()
  ) %>%
  arrange(desc(shipments))

# Professional Pharmacy Service, located at 2411 West Belvedere Avenue, received the most shipments - 6,254 in total. This pharmacy is located inside Sinai Hospital, which serves Baltimore City, Baltimore County, and the surrounding communities, which could help explain the number of shipments.
...
# Rite Aid of Maryland Inc. was sent 16,740,590 pills, 12.5% of Baltimore City's 133,721,503 total pills.
...
# There are three different iterations of Rite Aid of Maryland in this data set, which didn't group correctly because of differences in punctuation and the use of "T/A" in the buyer_name field.
...
baltimore %>%
  mutate(rite_aid_y_n = if_else(str_detect(buyer_name,"RITE"),"y","n")) %>%
  group_by(rite_aid_y_n) %>%
  summarise(shipments = n()) %>%
  mutate(total_shipments = sum(shipments)) %>%
  mutate(ship_pct = round((shipments/total_shipments)*100,2)) %>%
  arrange(desc(ship_pct))

# 290,908 shipments went to Baltimore City, 15.1% of which (43,920) went to Rite Aid stores.
...
baltimore %>%
  ...
  summarise(shipments = n(),
            total_pills = sum(dosage_unit),
            avg_pills = mean(dosage_unit),
            max_pills = max(dosage_unit)
  ) %>%
  arrange(desc(avg_pills))

# Most shipments: SpecGx LLC--RITE AID MID-ATLANTIC--RITE AID OF MARYLAND, INC. (9,087 shipments)
# Most pills: SpecGx LLC--RITE AID MID-ATLANTIC--RITE AID OF MARYLAND, INC. (2,482,900 pills)
# Highest average pills: Actavis Pharma, Inc.--CARDINAL HEALTH--NEWCARE HOME HEALTH SERVICES (67 shipments for 957,100 pills - approx. 14,285 pills per shipment) - Newcare responsible for 7 of the 10 highest averages per shipment
# Most pills in max shipment: Actavis Pharma, Inc.--CARDINAL HEALTH--NEWCARE HOME HEALTH SERVICES (48,000 pills in one shipment)

#Additional information that would be useful:
# --The pipelines with the largest max shipments - are these isolated large shipments, or is there a pattern of multiple huge ones?
# --More information about SpecGx LLC, who are in 7 of the top 10 pipelines with the most shipments
# --Is there something special about Actavis Pharma, who frequently deliver the highest average numbers of pills per shipment?
...
baltimore %>%
  ...
  filter(pipeline == "Actavis Pharma, Inc.--CARDINAL HEALTH--NEWCARE HOME HEALTH SERVICES") %>%
  select(pipeline, transaction_date)

# The Actavis Pharma, Inc.--CARDINAL HEALTH--NEWCARE HOME HEALTH SERVICES shipment of 48,000 pills took place on March 21, 2006.
...
# Question: Is NewCare making frequent large purchases of opioids?

baltimore %>%
  filter(buyer_name == "NEWCARE HOME HEALTH SERVICES") %>%
  select(reporter_name,transaction_date,dosage_unit) %>%
  arrange(desc(dosage_unit))

# Answer: Yes. NewCare received 29 shipments of at least 20,000 pills, and 92 of at least 10,000.

```

```{r}

# Question: Is NewCare's pattern of large purchases of opioids (as outlined in the previous question) unique?

baltimore %>%
  filter(dosage_unit >= 10000) %>%
  group_by(buyer_name) %>%
  summarise(shipments = n()) %>%
  arrange(desc(shipments))

# Answer: Yes. Only HMIS (135 shipments with at least 10,000 pills) received more large shipments, and the next buyer on the list (Park Lane Pharmacy Inc.) had 13 such shipments.
