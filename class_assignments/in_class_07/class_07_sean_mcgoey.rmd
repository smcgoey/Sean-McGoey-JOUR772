...
pills_population_wv_county <- pills_population %>%
...
geo_pills_wv_county <- county_geodata %>%
 inner_join(pills_population_wv_county, by=c("GEOID" = "countyfips"))
...
geo_pills_wv_county %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Even as shipments decline statewide, Mingo and Logan counties consistently receive the most pills", subtitle = "Pills per person by county, 2006-2014", caption = "Source: DEA ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "viridis")

# Obviously the outliers (Mingo/Logan/Boone counties) pop out here, with clearly the most pills per person in every year. But one other thing that I find interesting is that there are a few counties in the northern part of the state that may have increased their pills per person in the later years, even as the statewide amounts decreased.
...
# Step 1: Filter the pills population table for West Virginia and Kentucky and create the pills per person metric.  
pills_population_wv_ky_county <- pills_population %>%
...
# Step 2: Join your pills per person table to a county level shapefile
geo_pills_wv_ky_county <- county_geodata %>%
 inner_join(pills_population_wv_ky_county, by=c("GEOID" = "countyfips"))

# Step 3: Draw the facet map
geo_pills_wv_ky_county %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Border counties in southeastern Kentucky and southwestern West Virginia receive the most pills", subtitle = "Pills per person by county in West Virginia and Kentucky, 2006-2009", caption = "Source: DEA ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "plasma")

# 2008 was obviously the peak year here, with the hottest hot spots near the KY/WV border. The fact that the hot spots also run along the border with Virginia is interesting as well - southwestern VA may merit some further looking into.
