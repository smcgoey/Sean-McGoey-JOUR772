...
# Glimpse mingo to see the columns and get a sense of what is contained in them
glimpse(mingo)
...
# Summarize mingo to get information about the values contained in each column
summary(mingo)
...
# View mingo in a new tab in the workspace - comment out once checked
#View(mingo)
...
mingo_working <- mingo %>%
  mutate(buyer_address_complete = paste0(buyer_address1," ", buyer_city, ", ", buyer_state, " ", buyer_zip)) %>%
  mutate(distributor_address_complete = if_else(reporter_address2 == "null", paste0(reporter_address1," ", reporter_city, ", ", reporter_state, " ", reporter_zip), paste0(reporter_address1," ", reporter_address2, " ",reporter_city, ", ", reporter_state, " ", reporter_zip)))
...
# Label distributors by in WV vs. out of WV
mingo_working <- mingo %>%
...
#Add median pills per shipment
mingo %>%
  group_by(buyer_name) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit), 
            avg_pills_per_shipment = mean(dosage_unit),
            med_pills_per_shipment = median(dosage_unit),
            min_pills_in_shipment = min(dosage_unit),
            max_pills_in_shipment = max(dosage_unit)
            ) %>%
  arrange(desc(max_pills_in_shipment))
...
# Group by manufacturer, distributor, and pharmacy name, then calculate the total no. of pills and sort descending
...
#The most significant combination is Actavis distributing through Miami-Luken to Tug Valley Pharmacy, with 4,954,200 total pills.
...
mingo %>%
  ...
  group_by (reporter_name, reporter_address1, reporter_city, reporter_state) %>%
  summarise(total_pills = sum(dosage_unit))

# Turns out Cardinal Health, Cardinal Health, Inc. and Cardinal Health 110, LLC are all truly different distributors.
...
# Since I checked in my last task using the str_detect function and found that there were no distributors besides those three with "CARDINAL" in the name, I would most likely use mutate and case_when to build a new column called "reporter_company" that would fill in "CARDINAL HEALTH" if reporter_name contained "CARDINAL" and would merely pull in what was already in reporter_name otherwise. Then we could keep the reporter_name field in case we still wanted to examine location-to-location distribution at some point.
