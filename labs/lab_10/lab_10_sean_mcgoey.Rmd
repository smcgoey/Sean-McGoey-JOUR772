...
medicare_opioids <- read_csv("data/medicare_opioids_dirty.csv") %>%
  ...
  mutate(medicare_opioid_prescribing_rate = na_if(medicare_opioid_prescribing_rate, "*")) %>%
  mutate(medicare_urban_opioid_prescribing_rate = na_if(medicare_urban_opioid_prescribing_rate, "*")) %>%
  mutate(medicare_rural_opioid_prescribing_rate = na_if(medicare_rural_opioid_prescribing_rate, "*")) %>%
  separate(county, into=c("county","state"), sep=",") %>%
  mutate(state = tolower(state)) %>%
  mutate(county = tolower(county)) %>%
  mutate(county = str_remove(county, "county|parish|borough")) %>%
  mutate(county = str_trim(county)) %>%
  mutate(state_code = as.numeric(str_sub(fips, start=1L, end=2L))) %>%
