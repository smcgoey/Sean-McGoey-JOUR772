...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Median Household Income", y="Average Annual Pills Per Person", title="", caption = "Source: Census data and DEA ARCOS database, via Washington Post")
...
# What we can see is that the (negative) relationship between median household income and the number of pills per person, while statistically significant, is much weaker than the (positive) relationship between opioid pills per person and opioid death rate. We can eyeball that to a certain extent by looking at the lines of best fit in the two scatterplots: The slope of the trend line in the death rate scatterplot is much steeper than the slope of the same line in the income scatterplot.

# This stands up to intuitive reason; you would expect there to be more opioid deaths in places with more opioid pills, similarly to how the presence of more guns in an area makes it likely that there will be more gun-related deaths. The connection to income is much murkier. We've spent a lot of time focusing on some very poor counties in West Virginia with truly shocking numbers of pills per person, but there are over 3,000 counties/jurisdictions in the United States, so the mean number of pills per person in those poor Rust Belt counties is obviously offset quite a bit by counties in their income cohort that didn't receive big opioid shipments.
