...
# Create a table for Mingo County, WV
...
# Build initial graphic for Mingo County with placeholder titles
ggplot(mingo_county_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="royal blue") +
  labs(x="Year", y="Total pills", title="This will be the Mingo County graphic title", subtitle = "Total pills shipped to Mingo County, WV, by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)
...
# Final version with amended title - hell, let's save a web graphic for good measure
ggplot(mingo_county_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="dark orange") +
  labs(x="Year", y="Total pills", title="In Mingo County, W.Va., opioids skyrocketed \nin 2008, then returned to earth in the 2010s", subtitle = "Total pills shipped to Mingo County, WV, by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)
...
# Create slice for West Virginia 2012 data only
west_virginia_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, year, dosage_unit)

# Create population table for WV 2012
west_virginia_population_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, population)

# Join the two tables to create table of West Virginia counties with data for pills and population
...
# Build initial scatterplot - use placeholder title and adjust population threshold from 175,000 to 70,000 (keeps six largest counties)
ggplot(west_virginia_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="PLACEHOLDER TITLE GOES HERE", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(west_virginia_2012, population > 70000 | dosage_unit > 6000000))

#NOTE: saw that there was a lower-population county with a particularly high number of pills, so added the OR condition to the subset function so I could get it labeled too (turns out it was Logan County). -SNM

```

```{r}

# final version - give the graphic a proper title and save an image
ggplot(west_virginia_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="In West Virginia, Logan County and Raleigh County have \nparticularly high numbers of opioids relative to population", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(west_virginia_2012, population > 70000 | dosage_unit > 6000000))
ggsave("wv_population.png", plot=last_plot())
