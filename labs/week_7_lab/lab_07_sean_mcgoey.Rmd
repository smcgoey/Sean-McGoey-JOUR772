...
# 999 records in our population table without a countyfips+year match in our pills table (I get 1,285 -SNM)
not_in_pills <- arcos_county_population_per_year %>%
  anti_join(arcos_county_pills_per_year, by=c("countyfips","year"))

# 595 records in our pills table without a countyfips+year match in population table. (I get 778 -SNM)  
...
# The thing that jumps out to me is how many of the records in not_in_population have missing countyfips values. That could cause a problem when trying to join these tables because without the unique identifier, you could wind up running into join issues because of things like multiple states with the same county name. There are also records without county names, which means we're not getting a 100% full picture of the states we're looking at. Additionally, there are many records from U.S. territories and military installations - not necessarily a problem in itself, but something worth noting.
...
# GETTING ERROR MESSAGE ON LAPTOP: Error in dyn.load(file, DLLpath = DLLpath, ...) : unable to load shared object '/Library/Frameworks/R.framework/Versions/3.6/Resources/library/systemfonts/libs/systemfonts.so': dlopen(/Library/Frameworks/R.framework/Versions/3.6/Resources/library/systemfonts/libs/systemfonts.so, 6): Library not loaded: /opt/X11/lib/libfreetype.6.dylib Referenced from: /Library/Frameworks/R.framework/Versions/3.6/Resources/library/systemfonts/libs/systemfonts.so Reason: image not found

# NOTE: Error seems to be resolved when running through rstudio.cloud. Still troubleshooting why I'm getting the error on my local machine. -SNM
...
# Same as previous instance, error on local machine is resolved when running in rstudio.cloud. -SNM
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Population',title="West Virginia, Kentucky received the most opioid pills per person in 2009", subtitle = "2017 population, U.S. Census", caption = "Source: U.S. Census ACS and DEA ARCOS database via The Washington Post") +
  scale_fill_continuous(labels = comma) +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Opioid shipments across the country peak in 2011 and 2012 before declining", subtitle = "Pills per person, 2006-2014", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
# Build pills_population_county_2009 object
...
# Join to shapefile
geo_pills_population_county_2009 <- county_geodata %>%
...
# One thing that jumps out to me is that in addition to Appalachia, there are big clusters in Oklahoma/Arkansas and along the west coast (northern California up into Oregon). Upon closer inspection, major cities (New York, Los Angeles, Atlanta, Baltimore, Dallas, etc.) tend to have fewer pills per person than counties further out - they definitely have more people clustered together, which brings down pills per person, but there could be other factors. They might also face tougher DEA scrutiny, for example, or opioid use may just be generally lower in bigger cities.
...
# Build Maryland county object
pills_population_md_county <- pills_population %>%
...
# Join to shapefile
geo_pills_md_county <- county_geodata %>%
 inner_join(pills_population_md_county, by=c("GEOID" = "countyfips"))
...
geo_pills_md_county %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Rural counties (Allegany, Cecil, Kent, Wicomico, Worcester) consistently \nreceive the most pills per person in Maryland", subtitle = "Pills per person by county, 2006-2014", caption = "Source: DEA ARCOS database") +
  theme(legend.position="bottom") +
